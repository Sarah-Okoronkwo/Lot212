...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const webpUrl = story.media_type === 'image' ? getWebPUrl(story.media_url, 1200) : null;
  ...
  return {
    ...
    openGraph: {
      title: story.caption,
      description: story.subtext || story.caption,
      images: webpUrl ? [{ url: webpUrl, alt: imageAlt, width: 1200 }] : [],
      type: 'article',
    ...
    twitter: {
      ...
      title: story.caption,
      description: story.subtext || story.caption,
      images: webpUrl ? [webpUrl] : [],
    },
  };
}

// JSON-LD structured data component
function ArticleStructuredData({ story, date, slug }: { story: Story; date: string; slug: string }) {
  const imageAlt = story.alt_text || story.caption;
  const webpUrl = story.media_type === 'image' ? getWebPUrl(story.media_url, 1200) : null;
  const pageUrl = `https://lot212.vercel.app/stories/${date}/${slug}`;
  ...
  const jsonLd = {
    ...
    publisher: {
      ...
      logo: {
        '@type': 'ImageObject',
        url: 'https://lot212.vercel.app/favicon.ico',
      },
    },
    ...(webpUrl && {
      image: {
...
export default async function StorySlugPage({ params }: PageProps) {
  ...
  const imageAlt = story.alt_text || story.caption;
  const webpUrl = story.media_type === 'image' ? getWebPUrl(story.media_url, 1080) : null;
  ...
  return (
    <>
      <ArticleStructuredData story={story} date={date} slug={slug} />
      ...
      >
        ...
        {/* Card */}
        <div className="relative z-10 w-full max-w-lg mx-auto flex flex-col overflow-hidden" style={{ minHeight: '100vh', maxHeight: '100vh' }}>
            ...
            <div className="absolute top-4 left-4 flex items-center gap-2 z-10">
              <Link href="/">
                <div
                  ...
                  style={{ background: '#e8ff47', color: '#18181f', fontFamily: 'var(--font-dm-mono)' }}
                >
                  212
                </div>
              </Link>
          ...
          >
            ...
            {/* Caption */}
            <h1 className="text-white font-bold leading-tight" style={{ fontSize: 'clamp(20px, 5vw, 26px)' }}>
              {story.caption}
            </h1>

            {/* Subtext */}
            {story.subtext && (
              <p className="leading-relaxed" style={{ color: 'rgba(255,255,255,0.7)', fontSize: 'clamp(14px, 3.5vw, 16px)' }}>
                {story.subtext}
              </p>
            )}
            ...
            <Link
              ...
              style={{ background: '#e8ff47', color: '#18181f' }}
            >
              <div>
            ...
            {/* Back link */}
            <Link href="/" className="text-center text-xs transition-colors" style={{ color: 'rgba(255,255,255,0.3)', fontFamily: 'var(--font-dm-mono)' }}>
              ← Back to Lot 212
            </Link>
          </div>
        </div>
